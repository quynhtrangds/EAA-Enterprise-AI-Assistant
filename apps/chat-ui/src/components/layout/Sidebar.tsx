    ...
    <div className={`bg-[#0f172a] relative z-50 text-slate-300 flex flex-col h-screen shadow-xl select-none transition-all duration-200 ease-in-out shrink-0 ${isOpen ? 'w-75' : 'w-[72px]'} border-r border-slate-800`}>
      ...
      <div className={`px-4 flex flex-col gap-2 mb-4 mt-2 shrink-0`}>
        ...
        <button
          ...
          disabled={isGuest}
          className={`flex items-center gap-3 px-2 h-10 w-full rounded-xl focus:outline-none transition-all duration-200 font-medium text-[13px] border border-transparent shrink-0 overflow-hidden ${isGuest
        ...
        >
          ...
          <span className={`truncate transition-all duration-200 text-left ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
            {isGuest ? 'Tìm kiếm trong các cuộc trò chuyện' : 'Tìm kiếm trong các cuộc trò chuyện'}
          </span>
        </button>
      </div>
